...
def main(args):
    ...
    with strategy.model_init_context():
        if args.model == 'bloom':
            ...
            actor.mark_only_lora_trainable()
        ...
        if rm_model_name == 'bloom':
            ...
            critic = ChatGLMCritic(pretrained=args.pretrain,lora_path=args.rm_lora_path,use_action_mask=False)
            critic.mark_only_lora_trainable()
    ...
    prompt_dataset = datasets.load_from_disk(args.prompt_path)
    ...
    pretrain_dataset = datasets.load_from_disk(args.pretrain_dataset)
    if dist.is_initialized() and dist.get_world_size() > 1:
